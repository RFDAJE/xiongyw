...
int main(int argc, char* argv[])
{
    ...
    /* 
     * print the header upto the first occurance of <glyph... 
     */
    get_next_glyph(p_input_file, &ucs2, &glyph_start, &size);
    ...
    for(p = p_input_file;;){
        ...
        /* check if we want to select this glyph */
...
EXIT:
     ...
     * cleanup and return 
     */
    munmap(p_input_file, input_stat.st_size);
    close(input_fd);
    return 0;
}
...
int get_next_glyph(const char* start,     /* input */
                   ...
                   char** glyph_start,    /* output: the exact start addr of the glyph */
                   size_t* glyph_size)    /* output: the size in bytes of the glyph */
{
