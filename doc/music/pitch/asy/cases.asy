/* created(bruin, 2007-03-09): draw cases with braces "{".
   $Id: cases.asy 2 2007-03-22 12:54:39Z Administrator $
 */
...
texpreamble("\usepackage{amssymb,amsmath,mathrsfs}
             ...
             %\DeclareRobustCommand{\cdash}{－\!\!\!－\!\!\!－}% 中文破折号
...
/* build node (and/or tree) in recursive fashion, e.g.,
...
or,
		node complex=node("复数", node("有理数"),
		                          node("无理数"));
		complex.kids[0].contains(new node[]{node("整数"), node("分数")});  
*/
	...
	if(root != null){
	  ...
	  //write("draw_tree" + root.text);
		...
		if(root.kids.length != 0){
			...
			//write(first_kid_ht_offset);
			//write(last_kid_ht_offset);
			//write(kids_ht);
			//write(center_y_adjust);	
